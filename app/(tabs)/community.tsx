...
const ALL_TAGS = ['Carleton', 'St. Olaf', 'In Town', 'Entertainment', 'Games', 'Crafts', 'Snack/Drink', 'Sports', 'Winter Sports',];
const locationalTags = ['Carleton', 'St. Olaf', 'In Town'];
const communityPartners: Partner[] = communityPartnersJson;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        data={partners.filter(partner => {
          ...
          const descriptiveTags = selectedTags.filter(tag => !locationalTags.includes(tag));
          const locationalSelectedTags = selectedTags.filter(tag => locationalTags.includes(tag));

          const hasAllDescriptive = descriptiveTags.every(tag => partnerTags.includes(tag));
          const hasAnyLocational = locationalSelectedTags.length === 0 || locationalSelectedTags.some(tag => partnerTags.includes(tag));

          return hasAllDescriptive && hasAnyLocational;
        })}
        renderItem={renderPartner}
        keyExtractor={(item, index) => index.toString()}
        contentContainerStyle={styles.listContainer}
